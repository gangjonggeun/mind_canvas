...
class StarrySeaSessionNotifier extends StateNotifier<HtpSessionEntity?> {
  ...
  Future<void> _initialize() async {
    // ... (초기화 로직 HTP와 동일, 생략 가능하지만 안전하게 복사 추천)
    await Hive.initFlutter();
    _box = await Hive.openBox<String>(_boxName);
    final appDir = await getApplicationDocumentsDirectory();
    _imageDir = Directory('${appDir.path}/starry_sea_temp'); // 폴더 분리
    if (!await _imageDir.exists()) await _imageDir.create(recursive: true);
    _loadSession();
  }
  ...
  // 👇 [변경] 별바다는 1개만 그리면 완료
  bool get canComplete => (state?.drawings.length ?? 0) >= 1;
...
/// Provider 정의
final starrySeaSessionProvider = StateNotifierProvider<StarrySeaSessionNotifier, HtpSessionEntity?>(
