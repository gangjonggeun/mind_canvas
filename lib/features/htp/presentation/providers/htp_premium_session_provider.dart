...
/// - 로컬 저장/복원
/// - 진행 상태 추적
class HtpPremiumSessionNotifier extends StateNotifier<HtpSessionEntity?> {
  static const String _boxName = 'htp_premium_current_session';
  ...
  HtpPremiumSessionNotifier() : super(null) {
    _initialize();
  }
  ...
  Future<void> _initialize() async {
    try {
      // 1. 🚨 수정됨: main에서 열어둔 박스를 즉시 가져옵니다 (await 제거)
      _box = Hive.box<String>(_boxName);

      // 2. 이미지 디렉토리 설정
      final appDir = await getApplicationDocumentsDirectory();
      _imageDir = Directory('${appDir.path}/htp_premium_temp');
  ...
  bool get canComplete {
    ...
    final bool isDrawingComplete = state!.drawings.length == 4;
...
/// Provider 정의
final htpPremiumSessionProvider = StateNotifierProvider<HtpPremiumSessionNotifier, HtpSessionEntity?>(
      (ref) => HtpPremiumSessionNotifier(),
);

/// 편의 Provider들
final htpCompletedCountProvider = Provider<int>((ref) {
  return ref.watch(htpPremiumSessionProvider.notifier).completedCount;
});

final htpCanCompleteProvider = Provider<bool>((ref) {
  return ref.watch(htpPremiumSessionProvider.notifier).canComplete;
});